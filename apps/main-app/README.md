...
- The implementation lives in [`libs/xstate-inspector`](../../libs/xstate-inspector/README.md), consumed as the `@io-app/xstate-inspector` workspace package: `src/` is the React Native bridge, `metro.js` is the Metro config plugin, `middleware.js` is the Metro bridge, and the UI it serves is the React app under `browser/src`, bundled by `browser/build.mjs` into `browser/dist`.
