...
/**
 * Position of the NFC antenna on the device, in millimeters from the top-left
 * corner.
 */
...
/**
 * Information about the NFC antennas available on the device. Includes the
 * dimensions (in millimeters) of the device to determine antenna placement.
 */
...
/**
 * Retrieves NFC antenna information on Android devices with API level 34+. The
 * information includes the device dimensions and the locations of available NFC
 * antennas in millimeters from the top-left corner of the device.
 *
 * @returns NFC antenna information.
 * @throws If the platform is not Android or if the native module fails to
 *   provide valid data.
 */
...
/**
 * Checks if Host Card Emulation (HCE) is supported on the device. HCE allows
 * the device to emulate an NFC card, enabling it to interact with NFC readers.
 *
 * @returns A promise that resolves to true if HCE is supported, or false if
 *   not.
 * @throws If the platform is not Android or if the native module fails to
 *   provide valid data.
 */
