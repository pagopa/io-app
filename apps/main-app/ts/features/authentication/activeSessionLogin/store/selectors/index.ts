...
export const cieLoginFlowSelector = (state: GlobalState): LoginType => {
  const activeSession = state.features.loginFeatures.activeSessionLogin;

  if (!activeSession?.isActiveSessionLogin) {
    return "auth";
  }

  if (activeSession.flow === "FCI") {
    return "FCI_auth";
  }
 ...
 *    remotely enabled AND the blocking screen hasn't been shown yet
 ...
 * scenarios:
 *
 * 1. Legacy mode: When active session login is NOT remotely enabled, but the
 *    session is expiring and the banner flag is true (maintains old behavior)
 * 2. Production mode: When the session is expiring, active session login is
 *    remotely enabled, the blocking screen has already been shown, and the
 *    banner flag is true
 * 3. Testing mode: When local testing is enabled, the blocking screen has been
 *    shown, and the banner flag is true (ignores session expiration for
 *    testing)
 *
