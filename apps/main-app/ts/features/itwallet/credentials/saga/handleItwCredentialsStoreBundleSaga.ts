 ...
 * credentialId:
 *
 * - The vault writes (one entry for a single credential, one per copy for a batch
 *   credential);
 * - The single metadata to store in Redux. A batch credential collapses into one
 *   metadata that lists all its copies' keyTags in `keyTags` (the first copy is
 *   the representative).
 */
...
/**
 * Collect and deduplicate by `uri` all status lists available in the provided
 * credential bundles. The output is then persisted via the
 * {@link StatusListRepository}.
 *
 * @param bundles The credential bundles to collect status lists from
 * @returns An array of status lists tuples with uri as key and the parsed list
 *   as payload
 */
