...
/**
 * Boot-time safety net for one-time-use credential batches: renews every pool that is down to its
 * refill threshold, no matter how it got there.
 *
 * It backs up the eager trigger in `handleItwCredentialsConsumeInstanceSaga`: if the app is killed
 * right after a presentation, or the renewal fails because the device is offline, the pool is
 * still under threshold at the next start and gets picked up here.
 */
