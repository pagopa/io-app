/**
 * This file defines error types for the support bottom sheet
 * (useItwFailureSupportModal), specifically for cases that don't fall under the
 * existing IssuanceFailure and CredentialIssuanceFailure types. A new global
 * error type, ItwFailure, has been introduced to handle errors that occur
 * outside of the state machines, ensuring a unified structure for IT Wallet
 * related failures.
 */
...
/**
 * Enriched error with credential metadata. Used to augment errors thrown by
 * `io-react-native-wallet` with the credential configuration ID.
 */
