...
/**
 * Higher-Order Component that conditionally renders a given screen or an
 * offline failure screen.
 *
 * This HOC determines the application's effective offline state by checking: 1.
 * If `isConnected` selector returns `false` (definitely offline). 2. If
 * `isConnected` selector returns `undefined` (connectivity is unknown). 3. If
 * `offlineAccessReason` selector returns a defined reason (e.g., session
 * expired), even if `isConnected` might be `true`.
 *
 * If any of these conditions indicate an offline or error state, the HOC
 * renders the `OfflineFailureComponent`. Otherwise (if `isConnected` is `true`
 * and no `offlineAccessReason` is present), it renders the original `Screen`
 * component.
 *
 ...
 *   const ProfileScreenWithOfflineFallback =
 *     withOfflineFailureScreen(MyScreen);
 *
 ...
 *   functionality.
 * @returns A new component that conditionally renders either the original
 *   `Screen` or the `OfflineFailureComponent`.
 */
