 ...
 * reading phase, which is handled outside `io-react-native-wallet`. We need to
 * resume the authentication flow after reading the card, so here we save the
 * auth params obtained in the first step.
 */
...
/**
 * The CieContext contains information about the CIE capabilities and NFC status
 * on the device.
 */
...
/**
 * The MrtdPoPContext contains the parameters needed to perform the Proof of
 * Possession (PoP) flow for MRTD-based eID issuance.
 */
export type MrtdPoPContext = {
  ...
  /**
   * The callback URL to be used after the MRTD PoP flow from which we fetch the
...
export type Context = {
  /**
   * IT-Wallet technical specifications version. Dependending on the particular
   * user configuration, the issuance machine must be able to use different
   * versions. This is a local value used only during the issuance flow.
   */
  itwVersion: ItwVersion;
  /**
   * The mode of eID issuance. This determines the flow and actions available in
   * the eID issuance process. Defaults to "issuance" if not specified.
   */
  ...
  /**
   * The integrity key tag used to verify the integrity of the wallet instance
   * attestation. If this is provided the machine will skip the wallet instance
  ...
  /**
   * The authentication context, which contains the parameters needed to resume
   * the authentication flow after reading the CIE card.
   */
