...
/**
 * Authorization headers which contains the bearer token for the current
 * session.
 */
type AuthHeaders = {
...
/**
 * Adds the authorization headers to the request initialization object. It
 * merges the provided authorization headers with the existing headers.
 *
 * @param authHeaders - An object containing authorization headers to be added
 *   to the request.
 * @param init - An optional RequestInit object that may contain existing
 *   headers.
...
/**
 * Getter for the authorization headers for the wallet provider based on the
 * provided URL. If the URL's origin matches the wallet provider's base URL and
 * the user is logged in, it returns the Authorization header with the session
 * token. Otherwise, it returns an empty object.
 *
 * @param sessionToken - The session token bearer token to be added to the
 ...
 * @returns An object containing the Authorization header if conditions are met,
...
/**
 * Creates a fetch function for the wallet provider functions that adds the
 * Authorization header with the session token if the user is logged in and the
 * URL matches the wallet provider base URL.
 *
 * @param sessionToken - The session token bearer token to be added to the
 *   Authorization header
 * @param walletProviderBaseUrl - The wallet provider base URL
 * @param url - The URL object to check against the wallet provider base URL
 * @returns A fetch function that can be used to make requests to the wallet
 *   provider.
 ...
 * @returns An instance of the IT Wallet client configured with the provided
 *   base URL and fetch function.
 */
