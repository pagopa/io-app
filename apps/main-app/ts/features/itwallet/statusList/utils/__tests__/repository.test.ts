...
describe("repository", () => {
  ...
  describe("upsert and get", () => {
    it("persists an entry and retrieves it by URI", async () => {
      ...
      await StatusListRepository.upsert(makeUri(1), payload, RESOLVED_AT);
    ...
    it("overwrites an existing entry for the same URI", async () => {
      ...
      await StatusListRepository.upsert(makeUri(1), payload1, RESOLVED_AT);
      await StatusListRepository.upsert(
        makeUri(1),
        payload2,
        RESOLVED_AT + 1000
    ...
    it("keeps concurrently inserted entries visible without index write races", async () => {
      await Promise.all([
        StatusListRepository.upsert(makeUri(1), makePayload(1), RESOLVED_AT),
        StatusListRepository.upsert(makeUri(2), makePayload(2), RESOLVED_AT)
      ]);
