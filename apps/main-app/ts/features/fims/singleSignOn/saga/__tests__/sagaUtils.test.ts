...
describe("absoluteRedirectUrl", () => {
  ...
  test.each(scenarios)(
    ...
    ({ redirect, originalRequestUrl, expected }) => {
      expect(absoluteRedirectUrl(redirect, originalRequestUrl)).toBe(expected);
    }
  );
});
...
describe("foldNativeHttpClientResponse", () => {
  it("calls foldSuccess when type is success", () => {
    ...
    foldNativeHttpClientResponse(foldSuccess, foldFailure)(mockSuccessResponse);
  ...
  it("calls foldFailure when type is failure", () => {
    const foldSuccess = jest.fn();
    const foldFailure = jest.fn();
    foldNativeHttpClientResponse(foldSuccess, foldFailure)(mockFailureResponse);
...
describe("isFastLoginFailure", () => {
  ...
  test.each(scenarios)("returns $expected for $name", ({ code, expected }) => {
    expect(isFastLoginFailure({ ...mockFailureResponse, code })).toBe(expected);
  });
});
...
describe("followProviderRedirects", () => {
  ...
  it("returns the response as-is when there is no redirect and it is a failure", async () => {
    ...
    const result = await followProviderRedirects(httpClientConfig, fimsDomain);
