...
/**
 * Function that handles the activation of a CGN Calls the activation API
 * returning the next iteration for orchestration saga: 201 -> Request created
 * start polling with handleCgnStatusPolling saga. 202 -> There's already a
 * processing request. 401 -> Bearer token null or expired. 409 -> Cannot
 * activate the user's cgn because another updateCgn request was found for this
 * user or it is already active. 403 -> Cannot activate a new CGN because the
 * user is ineligible to get the CGN.
 *
