...
import { GetState, Dispatch, Action, Thunk } from './types'
import { ApiUserProfile, ApiNewUserProfile } from '../utils/api'
import { getUserProfile } from '../utils/api'
import { setUserProfile } from '../utils/api'

import I18n from '../i18n'

export const REQUEST_USER_PROFILE_ACTION = 'REQUEST_USER_PROFILE_ACTION'
export const RECEIVE_USER_PROFILE_ACTION = 'RECEIVE_USER_PROFILE_ACTION'
export const UPDATE_USER_PROFILE_REQUEST_ACTION = 'UPDATE_USER_PROFILE_REQUEST_ACTION'
export const UPDATE_USER_PROFILE_ERROR_ACTION = 'UPDATE_USER_PROFILE_ERROR_ACTION'

// Creators
export function receiveUserProfile(profile: ApiUserProfile): Action {
  return {
...
export function updateUserProfileError(): Action {
  return {
 ...
 */
export function requestUserProfile(): Thunk {
 ...
 */
export function updateUserProfile(newProfile: ApiNewUserProfile): Thunk {
  return (dispatch: Dispatch, getState: GetState) => {
    const user = getState().user
    // If the user is not logged in we can't request the profile
    if (!user.isLoggedIn) {
