 ...
 *
 * Get awaken by the receival of `startProcessingMessageArchivingAction` in
 * takeLastest mode do { Extract the first message ID for archiving/restoring
 * queues, along with the info about archiving or restoring the message If no
 * data was returned, then the process has ended. Dispatch
 * resetMessageArchivingAction with success Otherwise Given the message id, look
 * for it in the related message page (inbox or archived) If no message has been
 * found, then it was already processed somehow, just discard the ID from the
 * archiving/restoring queues (by dispatching
 * removeScheduledMessageArchivingAction) and continue to the next do-while loop
 * interaction Otherwise Build and dispatch the legacy
 * upsertMessageStatusAttributes.request. Such action triggers the legacy saga
 * which uses the legacy archiving/restoring logic to move the message to the
 * other list, contact the server and, in case of a failure, puts back the
 * message to the source list). Such saga is raced against
 * resetMessageArchivingAction in order to be cancelable Listen for the saga
 * async output action, which is either upsertMessageStatusAttributes.success or
 * upsertMessageStatusAttributes.success but also for
 * resetMessageArchivingAction to avoid a race conidtion where the saga fails
 * one moment before the user cancels the processing (dispatching
 * resetMessageArchivingAction by the UI button) If
 * upsertMessageStatusAttributes.failure is received, then cancel the processing
 * using interruptMessageArchivingProcessingAction (which does not clear the
 * user selection) and stop the current saga If
 * upsertMessageStatusAttributes.success is received, then discard the ID from
 * the archiving/restoring queues (by dispatching
 * removeScheduledMessageArchivingAction) and continue to the next do-while loop
 * interaction If resetMessageArchivingAction is received, then just go to the
 * next do-while loop interaction, which will extract no data from the
 * archiving/restoring queues (since they have been emptied by
 * resetMessageArchivingAction) and will terminate } while (true)
 *
