...
import { getPaymentsNoticeReceiptAction } from "../store/actions";
...
export function* handleGetReceipt(
  ...
  action: ActionType<(typeof getPaymentsNoticeReceiptAction)["request"]>
) {
  try {
    ...
    if (E.isLeft(getTransactionReceiptResult)) {
      ...
      yield* put(
        getPaymentsNoticeReceiptAction.failure({
          ...getGenericError(
    ...
    if (getTransactionReceiptResult.right.status === 200) {
      ...
      yield* put(
        getPaymentsNoticeReceiptAction.success({ base64File, filename })
      );
