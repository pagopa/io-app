...
import { getPaymentsLatestNoticeAction } from "../store/actions";
...
export function* handleGetLatestNotice(
  getTransactionList: TransactionClient["getPaidNotices"],
  action: ActionType<(typeof getPaymentsLatestNoticeAction)["request"]>
) {
  try {
    ...
    if (E.isLeft(getTransactionListResult)) {
      yield* put(
        getPaymentsLatestNoticeAction.failure({
          ...getGenericError(
