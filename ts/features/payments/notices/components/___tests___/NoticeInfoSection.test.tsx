...
describe("NoticeInfoSection", () => {
  it("renders loading skeletons when loading is true", () => {
    const { getAllByTestId } = render(<NoticeInfoSection loading={true} />);
  ...
  it("renders transaction information correctly when transaction data is provided", () => {
    const { getByText } = render(
      <NoticeInfoSection transaction={mockTransaction} loading={false} />
    );
  ...
  it("does not render sections if transaction data is missing", () => {
    const { queryByText } = render(<NoticeInfoSection loading={false} />);
    expect(
  ...
  it("renders PayPal instead of credit card if only maskedEmail is provided", () => {
    ...
    const { getByText } = render(
      <NoticeInfoSection transaction={paypalTransaction} loading={false} />
    );
