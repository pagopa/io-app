import { createStackNavigator } from "@react-navigation/stack";
import LogoutScreen from "../views/LogoutScreen";
import { DesignSystemNavigator } from "../../../design-system/navigation/navigator";
import LollipopPlayground from "../../../lollipop/playgrounds/LollipopPlayground";
import CalendarsPreferencesScreen from "../../preferences/views/CalendarsPreferencesScreen";
import CgnLandingPlayground from "../components/playgrounds/CgnLandingPlayground";
import DownloadProfileDataScreen from "../../privacy/views/DownloadProfileDataScreen";
import EmailForwardingScreen from "../../preferences/views/EmailForwardingScreen";
import EmailInsertScreen from "../../userData/shared/views/EmailInsertScreen";
import EmailValidationSendEmailScreen from "../../userData/shared/views/EmailValidationSendEmailScreen";
import FiscalCodeScreen from "../../userData/views/FiscalCodeScreen";
import LanguagesPreferencesScreen from "../../preferences/views/LanguagesPreferencesScreen";
import { NotificationsPreferencesScreen } from "../../preferences/views/NotificationsPreferencesScreen";
import PinScreen from "../../security/views/PinScreen";
import PreferencesScreen from "../../preferences/views/PreferencesScreen";
import PrivacyMainScreen from "../../privacy/views/PrivacyMainScreen";
import ProfileAboutApp from "../../aboutApp/views/ProfileAboutApp";
import ProfileDataScreen from "../../userData/views/ProfileDataScreen";
import RemoveAccountDetails from "../../privacy/views/RemoveAccountDetailsScreen";
import RemoveAccountInfo from "../../privacy/views/RemoveAccountInfoScreen";
import RemoveAccountSuccess from "../../privacy/views/RemoveAccountSuccessScreen";
import SecurityScreen from "../../security/views/SecurityScreen";
import ServicesPreferenceScreen from "../../preferences/views/ServicesPreferenceScreen";
import ShareDataScreen from "../../privacy/views/ShareDataScreen";
import TosScreen from "../../privacy/views/TosScreen";
import { IdPayCodePlayGround } from "../components/playgrounds/IdPayCodePlayground";
import IdPayOnboardingPlayground from "../components/playgrounds/IdPayOnboardingPlayground";
import { isGestureEnabled } from "../../../../utils/navigation";
import TrialSystemPlayground from "../components/playgrounds/TrialSystemPlayground";
import ProfileMainScreen from "../views/ProfileMainScreen";
import { IOMarkdownPlayground } from "../components/playgrounds/IOMarkdownPlayground";
import { AppFeedbackPlayground } from "../components/playgrounds/AppFeedbackPlayground";
import AppearancePreferenceScreen from "../../preferences/views/AppearancePreferenceScreen";
import { SettingsParamsList } from "./params/SettingsParamsList";
import { SETTINGS_ROUTES } from "./routes";

const Stack = createStackNavigator<SettingsParamsList>();

/**
 * A navigator for all the screens of the Settings section
 */
const SettingsStackNavigator = () => (
  <Stack.Navigator
    initialRouteName={SETTINGS_ROUTES.SETTINGS_MAIN}
    screenOptions={{ gestureEnabled: isGestureEnabled, headerMode: "screen" }}
  >
    <Stack.Screen
      options={{ headerShown: true }}
      name={SETTINGS_ROUTES.SETTINGS_MAIN}
      component={ProfileMainScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_DATA}
      component={ProfileDataScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PRIVACY_MAIN}
      component={PrivacyMainScreen}
    />
    <Stack.Screen
      options={{ headerShown: true }}
      name={SETTINGS_ROUTES.PROFILE_PRIVACY}
      component={TosScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PRIVACY_SHARE_DATA}
      component={ShareDataScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_SECURITY}
      component={SecurityScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_SERVICES}
      component={ServicesPreferenceScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_APPEARANCE}
      component={AppearancePreferenceScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_EMAIL_FORWARDING}
      component={EmailForwardingScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_HOME}
      component={PreferencesScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_CALENDAR}
      component={CalendarsPreferencesScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_LANGUAGE}
      component={LanguagesPreferencesScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_ABOUT_APP}
      component={ProfileAboutApp}
    />
    <Stack.Screen
      options={{
        headerShown: false
      }}
      name={SETTINGS_ROUTES.PROFILE_LOGOUT}
      component={LogoutScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_FISCAL_CODE}
      component={FiscalCodeScreen}
    />
    <Stack.Screen
      options={{ gestureEnabled: false }}
      name={SETTINGS_ROUTES.INSERT_EMAIL_SCREEN}
      component={EmailInsertScreen}
    />
    <Stack.Screen
      options={{ gestureEnabled: false, headerShown: false }}
      name={SETTINGS_ROUTES.EMAIL_VERIFICATION_SCREEN}
      component={EmailValidationSendEmailScreen}
    />
    <Stack.Screen name={SETTINGS_ROUTES.PIN_SCREEN} component={PinScreen} />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_DOWNLOAD_DATA}
      component={DownloadProfileDataScreen}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.IO_MARKDOWN_PLAYGROUND}
      component={IOMarkdownPlayground}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.APP_FEEDBACK_PLAYGROUND}
      component={AppFeedbackPlayground}
    />
    <Stack.Screen
      options={{
        headerShown: false
      }}
      name={SETTINGS_ROUTES.DESIGN_SYSTEM}
      component={DesignSystemNavigator}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.LOLLIPOP_PLAYGROUND}
      component={LollipopPlayground}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.CGN_LANDING_PLAYGROUND}
      component={CgnLandingPlayground}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.TRIALS_SYSTEM_PLAYGROUND}
      component={TrialSystemPlayground}
    />
    <Stack.Screen
      options={{
        headerShown: false
      }}
      name={SETTINGS_ROUTES.IDPAY_ONBOARDING_PLAYGROUND}
      component={IdPayOnboardingPlayground}
    />
    <Stack.Screen
      options={{
        headerShown: false
      }}
      name={SETTINGS_ROUTES.IDPAY_CODE_PLAYGROUND}
      component={IdPayCodePlayGround}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_REMOVE_ACCOUNT_INFO}
      component={RemoveAccountInfo}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_REMOVE_ACCOUNT_DETAILS}
      component={RemoveAccountDetails}
    />
    <Stack.Screen
      options={{
        headerShown: false
      }}
      name={SETTINGS_ROUTES.PROFILE_REMOVE_ACCOUNT_SUCCESS}
      component={RemoveAccountSuccess}
    />
    <Stack.Screen
      name={SETTINGS_ROUTES.PROFILE_PREFERENCES_NOTIFICATIONS}
      component={NotificationsPreferencesScreen}
    />
  </Stack.Navigator>
);

export default SettingsStackNavigator;
