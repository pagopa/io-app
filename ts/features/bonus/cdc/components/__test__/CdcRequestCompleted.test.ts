...
import CdcRequestCompleted from "../CdcRequestCompleted";
...
describe("CdcRequestCompleted", () => {
  it("Should show the CdcRequestCompleted screen with the title, the body and the button", () => {
    const component = renderComponent();
    expect(component.getByTestId("cdcRequestCompleted")).toBeDefined();
    expect(
      component.getByText(
        I18n.t("bonus.cdc.bonusRequest.bonusRequested.requestCompleted.title")
      )
    ).toBeDefined();
    expect(
      component.getByText(
        I18n.t("bonus.cdc.bonusRequest.bonusRequested.requestCompleted.body")
      )
    ).toBeDefined();
