...
/**
 * Unknown value const for FCI environment. The autogenerated
 * {@link EnvironmentEnum} does not contain a value for unknown. We need to add
 * it manually but we cannot add it to the autogenerated enum, nor we can extend
 * it. This is needed because when we dispatch the action to request a signature
 * detail we still don't know the environment as it is contained in the response
 * of the attached saga. It's also used when we don't have the environment in
 * the response due to an error.
 */
