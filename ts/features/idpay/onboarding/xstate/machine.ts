...
const getMultiRequiredCriteria = (context: Context) =>
  discernCriteria(context, SelfDeclarationMultiDTO.is);

export const getBoolRequiredCriteria = (context: Context) =>
  discernCriteria(context, SelfDeclarationBoolDTO.is);
...
const isNextMultiPageAvailable = (context: Context) => {
  ...
  return requiredCriteria[context.multiConsents?.length ?? 0] !== undefined;
};
        ...
        DISPLAYING_REQUIRED_SELF_CRITERIA: {
          ...
          states: {
            ...
            CYCLE_MULTI_CRITERIA: {
              tags: [WAITING_USER_INPUT_TAG],
              entry: "pushMultiSelfDeclarationPage",
              on: {
                ADD_SELF_CONSENT: [
                  {
                    cond: "isNextMultiPageAvailable",
                    actions: ["addSelfConsent", "pushMultiSelfDeclarationPage"]
                  },
                  {
    ...
    },
    {
      actions: {
        ...
        addSelfConsent: assign((_, event) => ({
          multiConsents: [...(_.multiConsents ?? []), event.data]
        }))
      },
      guards: {
        isOnboardingDone,
