...
export function* handleGetInitiativePaymentMethods(
  ...
  action: ActionType<typeof idpayInitiativePaymentMethodsGet["request"]>
) {
  ...
  try {
    ...
    yield pipe(
      ...
      E.fold(
        error =>
          put(
            idpayInitiativePaymentMethodsGet.failure({
        ...
        response =>
          put(
            response.status === 200
              ? idpayInitiativePaymentMethodsGet.success(response.value)
              : idpayInitiativePaymentMethodsGet.failure({
                  ...getGenericError(
    ...
    yield* put(
      idpayInitiativePaymentMethodsGet.failure({ ...getNetworkError(e) })
    );
  }
}
