...
describe("IDPay Payment machine", () => {
  ...
  it("should show failure toast if authorization fails with non-blocking error", async () => {
    const mockPreAuthorizePayment = jest.fn(async () =>
      Promise.resolve(T_TRANSACTION_DATA_DTO)
    );
    const mockAuthorizePayment = jest.fn(async () =>
      Promise.reject(PaymentFailureEnum.TOO_MANY_REQUESTS)
    );
    ...
    await waitFor(() =>
      expect(mockNavigateToResultScreen).toHaveBeenCalledTimes(0)
    );
