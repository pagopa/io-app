import { deleteKey, sign } from "@pagopa/io-react-native-crypto";
import {
  decodeAssertion,
  generateHardwareKey,
...
import { INTEGRITY_ANDROID_KEYTAG } from "./itwCryptoContextUtils";
...
const generateIntegrityHardwareKeyTag = () =>
  Platform.select({
    ...
    android: async () => {
      // Here we delete the existing key, if any.
      // However, we always return a constant since the key is generated by getAttestation method of io-react-native-integrity package.
      await deleteKey(INTEGRITY_ANDROID_KEYTAG);
      return INTEGRITY_ANDROID_KEYTAG;
    },
