...
it("should dispatch itwSetFiscalCodeWhitelisted(true) on success response with whitelisted true", () => {
  ...
  testSaga(handleGetWhitelistedStatus, mockItWalletClient as ItWalletClient, sessionToken as SessionToken)
    .next()
    .call(mockItWalletClient.isFiscalCodeWhitelisted!, { Bearer: sessionToken })
    .next(response)
