 ...
 */
export function getIdpInfo(idpId: string): IdentityProvider | undefined {
...
> {
  ...
  createButtons = (): ReadonlyArray<React.ReactNode> => {
    return enabledIdps.map((idp: IdentityProvider) =>
  ...
  createDemoButton(): React.ReactNode {
    return this.createButton(demoIdp, () => {
      this.props.onSelectIdp(demoIdp)
    })
  }

  createErrorMessage = (): React.ReactNode => {
    return (
...
export type SpidLoginButtonState = {
