...
/**
 * Load the services detail not yet stored.
 * A service detail has to be loaded if one of these conditions is true:
 * - any service is stored for the service id (servicesByIdSelector)
 * - the stored value associated to the service id is a pot none (but not loading)
 * - the stored value associated to the service id is some and its version is less than the visible one
