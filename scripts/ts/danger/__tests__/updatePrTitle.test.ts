...
describe("changelog", () => {
  const featTicket = getJiraTicketExample("Story");
  const choreTicket = getJiraTicketExample("Sub-task");
  const featFciTicket = getJiraTicketExample("Story", "SFEQS");
  const featIdPayTicket = getJiraTicketExample("Story", "IODPAY");

  describe("allStoriesSameType", () => {
    it("should return true if all stories have the same type", () => {
      const stories: Array<RemoteJiraTicket> = [featTicket, featTicket];
      const result = allStoriesSameType(stories);
      expect(result).toBe(true);
    });

    it("should return false if stories have different types", () => {
      const stories: Array<RemoteJiraTicket> = [featTicket, choreTicket];
      const result = allStoriesSameType(stories);
  ...
  describe("getTicketChangelogScope", () => {
    it("should return the changelog scope if the story belongs to a scope project", () => {
      const result = getTicketChangelogScope(featFciTicket);
      expect(O.isSome(result)).toBe(true);
      expect(O.getOrElse(() => "")(result)).toBe("Firma con IO");
    });

    it("should return None if the story doesn't belong to a scope project", () => {
      const result = getTicketChangelogScope(featTicket);
  ...
  describe("getChangelogScope", () => {
    ...
    it("should return Right(Some(scope)) if one of the stories have a scope or all the stories that have scope have the same scope", () => {
      const stories: Array<RemoteJiraTicket> = [featFciTicket, featFciTicket];
      const result = getChangelogScope(stories);
      pipe(
        result,
        E.fold(
          ...
          scope => {
            ...
            expect(O.getOrElse(() => "")(scope)).toBe("Firma con IO");
          }
        )
      );
    });
    ...
    it("should return Left(errors) if two stories have different scope or one of the story have different scope", () => {
      const stories: Array<RemoteJiraTicket> = [featFciTicket, featIdPayTicket];
      const result = getChangelogScope(stories);
