...
if [ $VAR_LENGTH -eq "0" ]; then
    ...
    channel="#io_status"
    res=$(curl -s -X POST -H 'Content-type: application/json' -H 'Authorization: Bearer '${IO_APP_SLACK_TOKEN_PAGOPA_SPEC_CHECK:-}'' --data '{"text":"'"$SEND_MSG"'", "channel" : "'$channel'"}' https://slack.com/api/chat.postMessage)
fi
