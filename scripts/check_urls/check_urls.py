...
def read_file(files, black_list_url):
	...
	for path in files:
		with open(path, 'r') as f:
			...
			uri_list = list(filter(lambda f: f not in black_list_url, uri_list))
			for u in uri_list:
				if u in uri_map:
...
def test_availability(uri_to_test):
	"""
		...
		"""
	try:
		r = requests.get(uri_to_test, headers=HEADERS, timeout=MAX_TIMEOUT, verify=False)
		if r.ok:
			return None
...
if not run_test and __name__ == '__main__':
	...
	print(f"found and processing {len(all_uris)} uris using {cpu_count()} cpus")
	for uri in all_uris:
