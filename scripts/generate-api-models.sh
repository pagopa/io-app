...
for elem in "${apis[@]}"; do
    ...
    echo ${strarr[0]}; rm -rf ${strarr[0]}; mkdir -p ${strarr[0]}; pnpm gen-api-models --api-spec ${strarr[1]} --out-dir ${strarr[0]} --no-strict --response-decoders --request-types --client &
done
wait
...
for elem in "${apisNoClient[@]}"; do
  read -a strarr <<< "$elem"  # uses default whitespace IFS
  echo ${strarr[0]}; [ ! -d ${strarr[0]} ] && mkdir -p ${strarr[0]}; pnpm run gen-api-models --api-spec ${strarr[1]} --out-dir ${strarr[0]} &
done
wait
